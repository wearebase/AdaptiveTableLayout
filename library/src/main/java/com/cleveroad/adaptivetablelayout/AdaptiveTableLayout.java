...
//        mShadowHelper = new ShadowHelper(mLayoutDirectionHelper);
    ...
    public void resetScroll() {
        scrollBy(-getTableScrollX(), -getTableScrollY());
    }

    public void scrollToPosition(int x, int y) {
        resetScroll();

        int xPos = mManager.getColumnsWidth(0, x);
        int yPos = mManager.getRowsHeight(0, y);
        scrollTo(xPos, yPos);
    }
    ...
    private void refreshHeaderColumnViewHolder(ViewHolder holder) {
        ...
        if (holder.isDragging()) {
            View leftShadow = null;//mShadowHelper.getLeftShadow();
            View rightShadow = null;//mShadowHelper.getRightShadow();
